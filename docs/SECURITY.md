...
- Each writable root receives a short-lived random capability-SID write ACE. Because Windows treats `DELETE` as a standard right outside `FILE_GENERIC_WRITE`, read-only roots also receive a temporary inherited `DELETE | FILE_DELETE_CHILD` deny ACE for the normal execution SID. Nested writable roots are temporarily protected from that inheritance when deletion is allowed. `DeletionPolicy.DENY` and protected paths receive the same normal-SID deletion deny in addition to their capability-SID rules. Original DACLs and inheritance flags are restored after execution while a cross-process ACL lease prevents stale-snapshot races. The target token uses `DISABLE_MAX_PRIVILEGE | WRITE_RESTRICTED` and deliberately omits the real user SID from its restricting list.
