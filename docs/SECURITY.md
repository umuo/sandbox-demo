...
- Writable roots receive a short-lived random synthetic SID Modify ACE. That SID is placed in a token created with `DISABLE_MAX_PRIVILEGE | WRITE_RESTRICTED`. The restricting list also contains the current logon-session SID and Windows' `S-1-5-33` Write Restricted Code compatibility SID, and those restricting SIDs are added to the token default DACL so the target can use per-logon and kernel objects required during process initialization. These compatibility SIDs grant nothing by themselves: the normal account access check must still pass, and the target can use them only on objects whose DACL explicitly grants a matching SID. `LUA_TOKEN` is deliberately not used because it is UAC filtering rather than the capability-SID boundary. Protected roots receive a write/delete deny ACE for the current capability SID.
