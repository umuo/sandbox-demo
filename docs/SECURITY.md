...
- Writable roots receive a short-lived random synthetic SID Modify ACE. That SID is placed in a token created with `DISABLE_MAX_PRIVILEGE | WRITE_RESTRICTED`. `LUA_TOKEN` is deliberately not used because it is UAC filtering rather than the capability-SID boundary, and its process-startup compatibility depends on UAC-specific token state. Protected roots receive a write/delete deny ACE for the current capability SID.
