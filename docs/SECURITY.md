...
- Writable roots receive a short-lived random synthetic SID Modify ACE. That SID is placed in a token created with `DISABLE_MAX_PRIVILEGE | WRITE_RESTRICTED`. The restricting list also contains Everyone (`S-1-1-0`), the current logon-session SID and Windows' `S-1-5-33` Write Restricted Code compatibility SID, and those restricting SIDs are added to the token default DACL so the target can use shared, per-logon and kernel objects required by general Win32/.NET process initialization. These compatibility SIDs grant nothing unless the normal account access check also passes and the object's DACL has a matching ACE. Consequently, a host location writable by Everyone remains writable; production hosts must not expose security-sensitive world-writable locations to the dedicated accounts. `LUA_TOKEN` is deliberately not used because it is UAC filtering rather than the capability-SID boundary. Protected roots receive a write/delete deny ACE for the current capability SID.
