...
- Writable roots receive a short-lived random synthetic SID Modify ACE. That SID is placed in a token created with `DISABLE_MAX_PRIVILEGE | LUA_TOKEN | WRITE_RESTRICTED`. Protected roots receive a write/delete deny ACE for the current capability SID.
...
- The worker verifies its actual account SID and consumes a sealed, bounded, versioned request file from a protected session directory.
